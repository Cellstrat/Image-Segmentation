...
def evaluateModel(args, model, up, image_list):
    ...
    for i, imgName in enumerate(image_list):
        ...
        if args.colored:
            ...
            print(args.savedir + os.sep + 'c_' + name.replace(args.img_extn, 'png'))
            cv2.imwrite(args.savedir + os.sep + 'c_' + name.replace(args.img_extn, 'png'), classMap_numpy_color)
            #cv2.imwrite('./results.res.png'', classMap_numpy_color)
            if args.overlay:
        ...
        cv2.imwrite(args.savedir + '/' + name.replace(args.img_extn, 'png'), classMap_numpy)
...
if __name__ == '__main__':
    ...
    # Run CPU version as Torch is installed with CPU only
    # parser.add_argument('--gpu', default=True, type=bool, help='Run on CPU or GPU. If TRUE, then GPU.')
    parser.add_argument('--gpu', default=False, type=bool, help='Run on CPU or GPU. If TRUE, then GPU.')
